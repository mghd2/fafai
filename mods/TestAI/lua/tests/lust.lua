...
function lust.it(name, fn)
  ...
  LOG(indent() .. color .. label .. normal .. ' ' .. name)
  if err then
    LOG(indent(lust.level + 1) .. red .. err .. normal)
  end
...
local paths = {
  [''] = { 'to', 'to_not' },
  to = { 'have', 'equal', 'exceed', 'be', 'exist', 'fail' },
  to_not = { 'have', 'equal', 'exceed', 'be', 'exist', 'fail', chain = function(a) a.negate = not a.negate end },
  a = { test = isa },
  an = { test = isa },
  be = { 'a', 'an', 'truthy',
    test = function(v, x)
  ...
  exceed = {
    test = function(v, x)
      return v > x,
        'expected ' .. repr(v) .. ' to exceed ' .. repr(x),
        'expected ' .. repr(v) .. ' to not exceed ' .. repr(x)
    end
  },
  have = {
    test = function(v, x)
